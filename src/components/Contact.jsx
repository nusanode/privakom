...
const Contact = () => {
  ...
  const prevSlide = () => {
    setActiveMapIndex((prev) => (prev === 0 ? contactData.locations.length - 1 : prev - 1));
  };

  const nextSlide = () => {
    setActiveMapIndex((prev) => (prev === contactData.locations.length - 1 ? 0 : prev + 1));
  };

  return (
    <section id="contact" className="px-6 lg:px-12 max-w-8xl mx-auto py-24 w-full bg-white">
      
      {/* Container Utama bergaya Brutalist */}
      <div className="bg-white border-[4px] border-dark shadow-[16px_16px_0px_0px_rgba(17,17,17,1)] flex flex-col md:flex-row relative">
        ...
        <div className="bg-dark text-white p-10 lg:p-14 md:w-5/12 flex flex-col justify-between border-b-[4px] md:border-b-0 md:border-r-[4px] border-dark relative overflow-hidden">
          <div className="z-10 relative">
            ...
            <div className="flex items-center gap-4 mb-6">
              ...
              <span className="text-primary font-bold tracking-widest text-sm uppercase">Our Locations</span>
            </div>
            ...
            <p className="text-gray-300 font-light text-xl mb-12 leading-relaxed">
              Find our nearest learning center or contact us directly. Click on the location below to see the map.
            </p>

            <div className="space-y-6">
              
              {/* Render Lokasi Cabang Dinamis */}
              {contactData.locations.map((branch, index) => (
                ...
                >
                  <div className={`p-3 border-[2px] shrink-0 transition-colors ${
                    activeMapIndex === index ? 'bg-dark border-dark text-primary' : 'bg-transparent border-gray-500 text-gray-400 group-hover:border-primary group-hover:text-primary'
                  }`}>
                  ...
                  <div>
                    <h4 className={`font-black font-serif text-2xl uppercase tracking-widest mb-2 transition-colors ${activeMapIndex === index ? 'text-dark' : 'text-white'}`}>
                      Privakom {branch.city}
                    </h4>
                    <p className={`text-sm leading-relaxed mb-4 line-clamp-2 font-medium ${activeMapIndex === index ? 'text-dark/80' : 'text-gray-400'}`}>
                      {branch.address}
                    </p>
                    
                    <div className="flex items-center gap-2 font-bold">
                      <svg className={`w-5 h-5 ${activeMapIndex === index ? 'text-dark' : 'text-primary'}`} fill="currentColor" viewBox="0 0 24 24">
        ...
        <div className="md:w-7/12 bg-light relative p-6 lg:p-10 min-h-[500px] lg:min-h-full flex items-center justify-center z-10">
          ...
          <div className="relative w-full h-[400px] lg:h-full border-[4px] border-dark shadow-[12px_12px_0px_0px_rgba(17,17,17,1)] overflow-hidden bg-dark">
            ...
            >
              {contactData.locations.map((branch) => (
                <div key={branch.id} className="w-full h-full flex-shrink-0 relative">
                  
                  {/* Badge Label Kota (Gaya Kotak Solid) */}
                  <div className="absolute top-6 right-6 z-20 bg-primary border-[3px] border-dark px-5 py-3 shadow-[6px_6px_0px_0px_rgba(17,17,17,1)] flex items-center gap-3 pointer-events-none">
                    ...
                    <span className="font-black text-dark text-sm uppercase tracking-widest">Privakom {branch.city}</span>
                  </div>
            ...
            {/* Tombol Panah Navigasi Slider (Gaya Kotak/Brutalist) */}
            <button 
              onClick={prevSlide}
            ...
            {/* Indikator Titik (Kotak) di bawah peta */}
            <div className="absolute bottom-8 left-1/2 -translate-x-1/2 flex items-center gap-3 z-20 bg-white border-[3px] border-dark px-4 py-3 shadow-[4px_4px_0px_0px_rgba(17,17,17,1)]">
              {contactData.locations.map((_, index) => (
                <button
                  key={index}
                  onClick={() => setActiveMapIndex(index)}
                  className={`transition-all duration-300 border-[2px] border-dark ${
                    activeMapIndex === index ? 'w-8 h-4 bg-primary' : 'w-4 h-4 bg-white hover:bg-gray-200'
                  }`}
